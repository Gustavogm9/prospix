...
export default function UserManagement() {
  ...
  const fetchUsers = async (newOffset = 0) => {
    ...
    try {
      ...
      const message = err instanceof AxiosError ? err.response?.data?.message || 'Falha ao carregar usuários.' : 'Falha ao carregar usuários.';
  ...
  const handleCreate = async () => {
    if (!createForm.tenantId || !createForm.name || !createForm.email) {
      toast.error('Erro', 'Tenant, nome e email são obrigatórios.');
      return;
    }
    ...
    try {
      ...
      toast.success('Usuário criado', `${user?.name} foi criado com sucesso.`);
      await fetchUsers(0);
    } catch (err: unknown) {
      const message = err instanceof AxiosError ? err.response?.data?.message || 'Falha ao criar usuário.' : 'Falha ao criar usuário.';
      toast.error('Erro', message);
    } finally {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      toast.success('Atualizado', 'Usuário atualizado com sucesso.');
  ...
  const handleResetPassword = async (userId: string) => {
    if (!confirm('Resetar senha deste usuário? Todas as sessões ativas serão revogadas.')) return;
    setBusyId(userId);
    try {
      ...
      toast.success('Senha resetada', 'Nova senha temporária gerada.');
    } catch (err: unknown) {
      const message = err instanceof AxiosError ? err.response?.data?.message || 'Falha.' : 'Falha.';
      toast.error('Erro', message);
    } finally {
  ...
  const handleDeactivate = async (userId: string) => {
    if (!confirm('Desativar este usuário? Ele será removido do sistema e todas as sessões serão revogadas.')) return;
    setBusyId(userId);
    try {
      await adminApiClient.delete(`/admin/users/${userId}`);
      toast.success('Desativado', 'Usuário desativado com sucesso.');
      await fetchUsers(pagination.offset);
    } catch (err: unknown) {
      const message = err instanceof AxiosError ? err.response?.data?.message || 'Falha.' : 'Falha.';
      toast.error('Erro', message);
  ...
  const handleReactivate = async (userId: string) => {
    ...
    try {
      ...
      toast.success('Reativado', 'Usuário reativado com sucesso.');
      await fetchUsers(pagination.offset);
    } catch (err: unknown) {
      const message = err instanceof AxiosError ? err.response?.data?.message || 'Falha.' : 'Falha.';
      toast.error('Erro', message);
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary text-xs mt-1">
            CRUD completo cross-tenant · criar, editar, resetar senha, desativar/reativar
          </p>
        </div>
      ...
      {/* Total count */}
      <p className="text-xs text-text-secondary">{pagination.total} usuário(s) encontrado(s)</p>

      {/* User list */}
      <Card className="bg-white border-border shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-bold font-heading text-text">Usuários</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-sm font-semibold text-text">Nenhum usuário encontrado.</p>
            </div>
          ) : (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-border">
                    <th className="text-left py-2 px-2 text-text-secondary font-semibold uppercase tracking-wider text-[10px]">Usuário</th>
                    ...
                    <th className="text-right py-2 px-2 text-text-secondary font-semibold uppercase tracking-wider text-[10px]">Ações</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((user) => {
                    ...
                    return (
                      <tr key={user.id} className={`border-b border-border/50 hover:bg-surface-sunken/30 transition-colors ${isDeactivated ? 'opacity-50' : ''}`}>
                        ...
                        <td className="py-2.5 px-2">
                          {user.tenant ? (
                            <Link to={`/tenants/${user.tenant.id}`} className="text-primary hover:underline font-medium">
                              {user.tenant.name}
                            </Link>
                          ) : (
                        ...
                        <td className="py-2.5 px-2 text-text-secondary font-mono text-[10px]">{user.whatsapp ?? '—'}</td>
                        <td className="py-2.5 px-2">
                          {isDeactivated ? (
                        ...
                        <td className="py-2.5 px-2">
                          ...
                          {resetResult?.userId === user.id && (
                            <div className="mt-1.5 p-2 bg-amber-50 border border-amber-200 rounded text-[10px]">
                              <span className="font-semibold text-amber-800">Senha temporária:</span>
          ...
          {pagination.total > PAGE_SIZE && (
            <div className="flex justify-between items-center mt-4 pt-3 border-t border-border">
              <span className="text-[10px] text-text-secondary">
                Mostrando {pagination.offset + 1}–{Math.min(pagination.offset + PAGE_SIZE, pagination.total)} de {pagination.total}
              </span>
              <div className="flex gap-1.5">
                <Button
            ...
            {tempPassword ? (
              <>
                <h3 className="text-lg font-bold font-heading text-text mb-4 flex items-center gap-2">
                  <UserCheck className="w-5 h-5 text-success-text" /> Usuário Criado!
                </h3>
                <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl mb-4">
                  <p className="text-xs text-amber-800 font-semibold mb-2">Senha Temporária (copie agora!):</p>
              ...
              <>
                <h3 className="text-lg font-bold font-heading text-text mb-4 flex items-center gap-2">
                  <Plus className="w-5 h-5 text-primary" /> Criar Novo Usuário
                </h3>

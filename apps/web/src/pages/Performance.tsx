    ...
    <div className="space-y-5 animate-fadeIn">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((card, i) => (
          <div key={i} className="bg-white border border-[#E5E7EB] rounded-xl p-4 shadow-sm">
            <div className="text-[11px] uppercase tracking-wider text-[#94A3B8] font-semibold mb-1.5">{card.label}</div>
            <div className="text-[26px] font-bold text-[#0F172A] font-mono tracking-tight leading-none">
              {card.value}
              {card.unit && <span className="text-[14px] text-[#94A3B8] font-sans font-medium">{card.unit}</span>}
            </div>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-xl shadow-sm overflow-hidden">
        <div className="px-5 py-3.5 border-b border-[#EEF0F3]">
          ...
          <div className="text-[11px] text-[#94A3B8] mt-0.5">Distribuição real de leads por estágio</div>
        </div>
        <div className="p-5 space-y-3">
          ...
          {funnelStages.length === 0 && (
            <div className="text-center text-[12px] text-[#94A3B8] py-6">Nenhum dado de funil disponível</div>
          )}
        </div>
      </div>
    </div>

...
// =========================================================================
// POST /api/admin/tenants/[id]/discovery/approve — Formal approval
// =========================================================================
  ...
  try {
    ...
    if (!current.approval_proof_r2_key) {
      return NextResponse.json(
        { message: 'approvalProofR2Key ausente. Faça upload do print de aprovação antes.' },
        { status: 409 },
      );
    }
    if (!current.pm_user_id) {
      return NextResponse.json(
        { message: 'pmUserId ausente. Atribua responsável antes de aprovar.' },
        { status: 409 },
      );
    }
    ...
    console.error('admin/discovery/approve → POST failed', err);
    return NextResponse.json({ message: 'Falha ao aprovar discovery.' }, { status: 500 });
  }
}

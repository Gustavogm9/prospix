...
// =========================================================================
// PUT /api/admin/tenants/[id]/discovery/scripts — Save scripts draft
// =========================================================================
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Scripts inválidos.' }, { status: 400 });
  }

  const scripts = body?.scripts;
  if (!scripts) {
    return NextResponse.json({ message: 'scripts é obrigatório.' }, { status: 400 });
  }
  ...
  try {
    ...
    if (existing) {
      const { data, error } = await supabaseAdmin
        .from('tenant_discoveries')
        .update({ scripts_draft: scripts as any })
        .eq('tenant_id', tenantId)
        .select()
        .single();
    ...
    console.error('admin/discovery/scripts → PUT failed', err);
    return NextResponse.json({ message: 'Falha ao salvar scripts.' }, { status: 500 });
  }
}

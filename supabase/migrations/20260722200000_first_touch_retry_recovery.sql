-- First-touch recovery layer.
-- Reopens only recoverable first-touch failures after the current canonical
-- checks still prove that the lead, campaign, script, phone, cadence window,
-- and Guardian relevance gates are safe.
  ...
  SELECT
    ...
    CASE
      ...
      WHEN COALESCE(retry.recoverable_failed_count, 0) > 0
        ...
        AND base.would_block_by_g02_relevance IS FALSE
        AND base.commercial_name_for_individual_script IS FALSE
        THEN 'ELIGIBLE'

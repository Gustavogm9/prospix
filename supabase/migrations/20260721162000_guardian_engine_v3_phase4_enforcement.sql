-- Guardian Engine V3 - Phase 4
-- Activates structural critical validators through a new active config version.
-- This migration is intentionally additive to history: previous active versions are archived,
-- copied, and superseded atomically instead of being edited in place.

CREATE EXTENSION IF NOT EXISTS pgcrypto;

CREATE TEMP TABLE _guardian_phase4_required_modes (
  guardian_key TEXT PRIMARY KEY,
  enforced_mode TEXT NOT NULL CHECK (enforced_mode IN ('BLOCK', 'HARD_BLOCK'))
) ON COMMIT DROP;

INSERT INTO _guardian_phase4_required_modes (guardian_key, enforced_mode) VALUES
  ('G04_IDENTITY_PERSONALIZATION', 'HARD_BLOCK'),
  ('G12_STRUCTURED_OUTPUT', 'HARD_BLOCK'),
  ('G13_PLACEHOLDER_LEAK', 'HARD_BLOCK'),
  ('G14_INTERNAL_LEAK', 'HARD_BLOCK');

CREATE TEMP TABLE _guardian_phase4_targets AS
SELECT
...
WHERE active_versions.status = 'ACTIVE'
  AND EXISTS (
    SELECT 1
    FROM _guardian_phase4_required_modes required_modes
...
SET status = 'ARCHIVED',
    notes = COALESCE(versions.notes || E'\n', '') || 'Archived by Guardian Engine V3 Phase 4 structural enforcement migration.'
FROM _guardian_phase4_targets targets
...
SELECT
  ...
  now(),
  now(),
  'Guardian Engine V3 Phase 4: G04, G12, G13 and G14 activated in HARD_BLOCK.'
FROM _guardian_phase4_targets targets;
...
LEFT JOIN _guardian_phase4_required_modes required_modes
...
SELECT
  ...
  targets.new_config_version_id,
  'Guardian Engine V3 Phase 4 structural critical validators activated.',
  now()
FROM _guardian_phase4_targets targets;

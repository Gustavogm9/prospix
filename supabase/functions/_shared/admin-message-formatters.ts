...
function buildReportSituation(counts: AdminCountMap, guardianRows: AdminGuardianStatus[], errors: string[]): string {
  ...
  const webhookIssues = countOf(counts, "webhookFailed") + countOf(counts, "webhookStaleProcessing");
...
function buildReportAction(counts: AdminCountMap, guardianRows: AdminGuardianStatus[], errors: string[]): string {
  ...
  if (webhookIssues > 0) return "Acao recomendada: verificar a entrada de mensagens recebidas e confirmar se alguma conversa deixou de atualizar.";
...
function buildWebhookProcessingLines(counts: AdminCountMap, webhookProcessing: AdminWebhookProcessing | undefined): string[] {
  ...
  if (repeated > 0) {
    lines.push(`Reenvios reconhecidos: ${repeated} ${plural(repeated, "evento repetido", "eventos repetidos")} sem duplicar conversa.`);
  }
  ...
  if (problemCount > 0 && issues.length > 0) {
    for (const issue of issues) {
      ...
      const summary = sanitizePreview(issue.operator_summary || issue.error_message || "entrada precisa de revisao", 180);
...
function formatQueueItem(item: AdminQueueItem): string {
  ...
  const attemptText = attempts > 0 ? `, ${attempts} ${plural(attempts, "tentativa", "tentativas")}` : "";
  return `- ${formatBrtHour(item.scheduled_for)}: ${queueTypeText(item.message_type)} para ${lead}${attemptText}.`;
}

function queueImpactLines(queue: AdminQueueImpact | null | undefined, fallbackDue: number | null | undefined): string[] {
  ...
  if (queue?.oldestDueAt) lines.push(`- Mais antiga vencida desde: ${formatBrtMinute(queue.oldestDueAt)}.`);
  if (queue?.nextScheduledFor) lines.push(`- Proxima prevista: ${formatBrtMinute(queue.nextScheduledFor)}.`);
...
export function buildAdminDisconnectAlertMessage(input: DisconnectMessageInput) {
  ...
  if (recentOutbound.length > 0) {
    bodyLines.push("", "Ultimos envios nas 5h anteriores:", ...recentOutbound.map(formatRecentMessage));
  } else {
    bodyLines.push("", "Ultimos envios nas 5h anteriores:", "Nao foram encontrados envios da IA nesse periodo.");
  }
...
export function buildAdminRecoveryStructuralAlertMessage(input: RecoveryStructuralAlertInput) {
  ...
  const body = bodyLines.join("\n");
  const summary = cleanText(`RECOVERY bloqueado por erro estrutural em ${tenantName}: ${structuralReason}`, 500);
  return { summary, body };
}

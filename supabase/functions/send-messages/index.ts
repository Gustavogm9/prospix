...
async function countFollowupsWithoutReply(
...
async function loadReusableFirstTouchConversation(
  tenantId: string,
  leadId: string,
): Promise<any | null> {
  const { data } = await supabase
...
async function claimDuePendingOutbound(params: {
  ...
  if (error) {
    console.warn(
      '  [Queue Claim] Falha ao reclamar pendencia atomicamente:',
...
async function acquireConversationLock(params: {
  ...
  const requestedLockUntil = new Date(
    Date.now() + Math.max(15, params.ttlSeconds) * 1000,
  ).toISOString();
...
function statusAfterHealthyExternalState(
  guardianStatus: any,
  externalState?: string | null,
): string {
...
async function countSuccessfulRecoverySends(
  tenantId: string,
...
async function countCriticalConnectionEvents(
  tenantId: string,
  sinceIso: string | null,
): Promise<number> {
...
async function triggerAdminMonitoringDue(source: string): Promise<void> {
  try {
    ...
    if (!response.ok) {
      ...
      console.warn(
        '[admin-monitoring] dispatcher returned HTTP ' +
          response.status +
          ': ' +
          redactText(text).slice(0, 240),
      );
    }
  } catch (err: any) {
    console.warn(
      '[admin-monitoring] dispatcher trigger failed:',
      redactText(err?.message || err).slice(0, 240),
    );
  }
}
...
function enqueueAdminMonitoringDue(source: string): void {
  ...
  promise.catch((err) => {
    console.warn(
      '[admin-monitoring] dispatcher fallback failed:',
      redactText(err?.message || err).slice(0, 240),
    );
  });
}
...
async function updateGuardianConnectionState(
  ...
  if (error) {
    console.warn(
      '  [WhatsApp Guard] Falha ao persistir estado completo:',
      redactText(error.message),
    );
    await recordConnectionEvent({
      tenantId,
    ...
    if (fallbackError) {
      console.warn(
        '  [WhatsApp Guard] Falha tambem no fallback de estado:',
...
async function runConnectionHealthGuard(
  ...
  if (external.ok) {
    ...
    if (String(previousStatus || '').toUpperCase() === 'RECOVERY') {
      ...
      const minSuccessfulSends = guardianNumber(
        recoveryGuardian,
...
function classifySendFailure(error: string): SendFailureClassification {
  ...
  if (errText.includes('device_removed'))
    return {
      critical: true,
      status: 'SUSPENDED',
      reasonCode: 'SEND_CRITICAL_DEVICE_REMOVED',
      shouldBackoff: false,
    };
  if (errText.includes('stream errored') || errText.includes('stream:error'))
    return {
      critical: true,
      status: 'SUSPENDED',
      reasonCode: 'SEND_CRITICAL_STREAM_ERRORED',
      shouldBackoff: false,
    };
  if (errText.includes('conflict'))
    return {
      critical: true,
      status: 'SUSPENDED',
      reasonCode: 'SEND_CRITICAL_SESSION_CONFLICT',
      shouldBackoff: false,
    };
  if (errText.includes('401') || errText.includes('unauthorized'))
    return {
      critical: true,
      status: 'SUSPENDED',
      reasonCode: 'SEND_CRITICAL_UNAUTHORIZED',
      shouldBackoff: false,
    };
  if (errText.includes('connection closed'))
    return {
      critical: true,
      status: 'PAUSED',
      reasonCode: 'SEND_CRITICAL_CONNECTION_CLOSED',
      shouldBackoff: false,
    };
  if (errText.includes('cannot read properties of undefined'))
    return {
      critical: true,
      status: 'PAUSED',
  ...
  if (errText.includes('instance does not exist') || errText.includes('not found'))
    return {
      critical: true,
      status: 'SUSPENDED',
      reasonCode: 'SEND_CRITICAL_INSTANCE_NOT_FOUND',
      shouldBackoff: false,
    };
  if (
    ...
    return {
      critical: false,
      status: 'PAUSED',
      reasonCode: 'SEND_TRANSIENT_TIMEOUT',
      shouldBackoff: true,
    };
  }
  if (
    ...
    return {
      critical: false,
      status: 'PAUSED',
      reasonCode: 'SEND_TRANSIENT_EVOLUTION_HTTP',
      shouldBackoff: true,
    };
  }
  ...
  return {
    critical: false,
    status: 'PAUSED',
    reasonCode: 'SEND_TRANSIENT_ERROR',
    shouldBackoff: false,
  };
}
...
async function generateIcebreaker(lead: any): Promise<string> {
  ...
  const cnaeDesc =
    lead.metadata?.cnpj_info?.cnae_principal?.descricao || lead.profession || 'empresa';
  const empresaNome =
    lead.metadata?.cnpj_info?.nomeFantasia || lead.metadata?.cnpj_info?.razaoSocial || lead.name;
...
async function sendWhatsApp(
  ...
  try {
    const timeoutMs = clampTimeoutMs(
      getNumberEnv('EVOLUTION_SEND_TIMEOUT_MS', 12000),
      12000,
      1000,
      60000,
    );
    ...
    return {
      ok: false,
      error: redactText(err?.message || err || 'EVOLUTION_SEND_UNKNOWN_ERROR').slice(0, 200),
    };
  }
}
...
function getGenderFromFirstName(name: string): 'M' | 'F' {
  ...
  if (
    cleanName.endsWith('y') &&
    !['wesley', 'valdecy', 'roney', 'rudy', 'darcy'].includes(cleanName)
  )
    return 'F';
...
async function substituteVariables(message: string, lead: any): Promise<string> {
  ...
  if (
    lead.metadata &&
    lead.metadata.cnpj_info &&
    lead.metadata.cnpj_info.qsa &&
    lead.metadata.cnpj_info.qsa.length > 0
  ) {
  ...
  const company =
    ...
    lead.metadata?.cnpj_info?.razaoSocial ||
    lead.name ||
    '';
  ...
  // Icebreaker Logic
  if (
    result.match(/(\[|\{)+Icebreaker(\]|\})+/gi) ||
    result.match(/(\[|\{)+Quebra-gelo(\]|\})+/gi)
  ) {
...
async function processFirstTouch(
  ...
  for (const campaign of campaigns) {
    ...
    if (!script) {
      ...
      if (scripts?.length) {
        const compatibleScripts = scripts.filter(
          (s: any) => !s.target_profession || s.target_profession === campaign.profession,
        );
        script =
          compatibleScripts.find((s: any) => s.target_profession === campaign.profession) ||
    ...
    if (processedLeadIds.size > 0) {
      eligibilityQuery = eligibilityQuery.not(
        'lead_id',
        'in',
        `(${Array.from(processedLeadIds)
          .map((id) => `'${id}'`)
          .join(',')})`,
      );
    }
    ...
    if (eligibilityError) {
      if (options.selectiveRetryOnly) {
        console.warn(
          '  [First Touch] Canonical eligibility view unavailable; RECOVERY legacy fallback blocked to avoid broad reprocessing.',
        );
        try {
          await dispatchAdminRecoveryStructuralAlert({
            ...
            structuralReason:
              'A retomada segura precisa da visao canonica de elegibilidade para selecionar somente leads realmente aptos. A consulta falhou, entao o sistema bloqueou o fallback legado para evitar disparos amplos ou duplicados.',
      ...
      console.warn(
        '  [First Touch] Canonical eligibility view unavailable, using legacy candidate query. Error: ' +
      ...
      const { data: legacyLeads } = await query.order('fit_score', { ascending: false }).limit(5);
      leads = legacyLeads || [];
      ...
      if (leadFetchError) {
        console.error(
          '  [First Touch] Erro ao carregar leads elegiveis canonicos: ' + leadFetchError.message,
        );
        failed++;
        continue;
      }
    ...
    for (const lead of leads) {
      ...
      const isTargetProfessionLiberal =
        scriptProfession.includes('médico') ||
        scriptProfession.includes('medico') ||
        scriptProfession.includes('doctor') ||
        scriptProfession.includes('doutor') ||
        scriptProfession.includes('advogado') ||
        scriptProfession.includes('lawyer') ||
        scriptProfession.includes('dentist') ||
        scriptProfession.includes('dentista') ||
        scriptProfession.includes('direito') ||
        scriptProfession.includes('saúde') ||
        scriptProfession.includes('saude');
      ...
      try {
        ...
        if (!firstTouchPreGenerationRun.allow) {
          const reasonCode =
            firstTouchPreGenerationRun.blockingDecision?.reason_code ||
            'GUARDIAN_PHASE5_FIRST_TOUCH_PRE_GENERATION_BLOCKED';
          ...
          await supabase.from('lead_events').insert({
            ...
            payload: {
              ...
              reason:
                'Guardian Engine V3 bloqueou primeira abordagem antes de gerar/enfileirar por relevancia ou estado.',
            },
            created_at: nowTime,
          });
          ...
          processedLeadIds.add(lead.id);
          console.warn(
            '  [Guardian V3] Primeira abordagem bloqueada antes da geracao. Lead: ' +
              lead.id +
              ' Reason: ' +
              reasonCode,
          );
        ...
        const firstTouchIdempotencyKey =
          firstTouchAttemptCount > 0
        ...
        if (!firstTouchGuardianRun.allow) {
          const reasonCode =
            firstTouchGuardianRun.blockingDecision?.reason_code ||
            'GUARDIAN_PHASE5_FIRST_TOUCH_BLOCKED';
          ...
          processedLeadIds.add(lead.id);
          console.warn(
            '  [Guardian V3] Primeira abordagem bloqueada antes da fila. Lead: ' +
              lead.id +
              ' Reason: ' +
              reasonCode,
          );
        ...
        let plannedScheduledFor = computeFirstResponseScheduledFor(
          g20Guardian,
          nowTime,
          `first-touch:${tenantId}:${lead.id}`,
        );
        let queueValidationStatus = 'APPROVED';
        let queueValidationReasonCode =
          firstTouchGuardianRun.summary.warn > 0
        ...
        if (firstTouchDelay) {
          ...
          const reasonCode =
            firstTouchPreEnqueueRun.blockingDecision?.reason_code ||
            'GUARDIAN_PHASE6_FIRST_TOUCH_PRE_ENQUEUE_BLOCKED';
          ...
          processedLeadIds.add(lead.id);
          console.warn(
            '  [Guardian V3] Primeira abordagem bloqueada no pre-enfileiramento. Lead: ' +
              lead.id +
              ' Reason: ' +
              reasonCode,
          );
        ...
        if (reusableConversation) {
          const { error: convErr } = await supabase
            ...
            .eq('tenant_id', tenantId);
          if (convErr)
            throw new Error('Erro ao reutilizar conversa para retentativa: ' + convErr.message);
        } else {
        ...
        const { error: queueErr } = await supabase.from('pending_outbound').insert({
          ...
          message_type: 'OUTBOUND_START',
          priority: 6,
          guardian_config_version_id:
            firstTouchPreEnqueueRun.configVersionId || firstTouchGuardianRun.configVersionId,
        ...
        const { error: variationErr } = await supabase
          ...
          .eq('id', variation.id);
        if (variationErr)
          throw new Error('Erro ao atualizar variação de script: ' + variationErr.message);
...
async function runGuardianWorkerForTenant(
  tenantId: string,
  runEndTime: number,
  ...
  if (lockErr || !lockUpdate || lockUpdate.length === 0) {
    console.log(
      '  🔒 [Lock Lógico] Worker para tenant ' + tenantId + ' já está rodando em outra instância.',
    );
    return { sent: 0, queued: 0, failed: 0 };
  }

  console.log(
    '  🚀 [Lock Lógico] Lock adquirido com sucesso para tenant ' +
      tenantId +
  ...
  try {
    while (Date.now() < runEndTime) {
      ...
      if (!healthDecision.allowSend) {
        console.log(
          '  [WhatsApp Guard] Envio bloqueado para tenant ' +
            tenantId +
            '. Motivo: ' +
      ...
      if (numberState === 'PAUSED' || numberState === 'SUSPENDED') {
        console.log(
          '  ⏸️ Guardião do tenant ' +
            tenantId +
            ' está pausado ou suspenso. Estado: ' +
            numberState,
        );
        await sleep(5000);
        continue;
      ...
      if (numberState === 'COLD') {
        ...
        const recoveryGuardian = getGuardianByKey(
          guardianConfig,
      ...
      try {
        ...
        if (!preGenerationGuardianRun.allow) {
          const reasonCode =
            preGenerationGuardianRun.blockingDecision?.reason_code ||
            'GUARDIAN_PHASE5_PRE_SEND_PRE_GENERATION_BLOCKED';
          ...
          await supabase
            ...
            .update({
              ...
              guardian_config_version_id:
                item.guardian_config_version_id || preGenerationGuardianRun.configVersionId,
            })
            .eq('id', item.id);
          ...
          await supabase.from('lead_events').insert({
            ...
            payload: {
              ...
              reason:
                'Guardian Engine V3 bloqueou mensagem ja enfileirada antes do envio por relevancia ou estado.',
            },
            created_at: new Date().toISOString(),
          });
          ...
          failed++;
          console.warn(
            '  [Guardian V3] Mensagem bloqueada antes do envio por relevancia/estado. Pending: ' +
              item.id +
              ' Reason: ' +
              reasonCode,
          );
        ...
        if (wakeSpreadDelay) {
          ...
          console.log(
            '  [Guardian V3] Mensagem reagendada por G18 para ' +
              wakeSpreadDelay.scheduledFor +
              '. Pending: ' +
              item.id,
          );
          await sleep(500);
        ...
        if (!preEnqueueGuardianRun.allow) {
          const reasonCode =
            preEnqueueGuardianRun.blockingDecision?.reason_code ||
            'GUARDIAN_PHASE6_PRE_ENQUEUE_BLOCKED';
          ...
          failed++;
          console.warn(
            '  [Guardian V3] Mensagem bloqueada no pre-enfileiramento tardio. Pending: ' +
              item.id +
              ' Reason: ' +
              reasonCode,
          );
        ...
        const [lastOutboundSentAt, followupCountWithoutReply, activeContacts30m] =
          await Promise.all([
        ...
        const conversationLockTtlSeconds = guardianNumber(
          g21Guardian,
        ...
        if (!conversationLock.acquired) {
          ...
          console.log(
            '  [Guardian V3] Conversation lock ocupado. Mensagem reagendada sem envio. Pending: ' +
              item.id,
          );
          await sleep(500);
          continue;
        ...
        try {
          ...
          try {
            ...
            const { error: finalGuardErr } = await supabase
              .from('pending_outbound')
              .update({
                guardian_config_version_id:
                  item.guardian_config_version_id || preSendGuardianRun.configVersionId,
                validation_status: 'APPROVED',
                validation_reason_code:
                  preSendGuardianRun.summary.warn > 0
                    ? 'GUARDIAN_PHASE6_PRE_SEND_WARN_OBSERVED'
                    : 'GUARDIAN_PHASE6_PRE_SEND_PASS',
                final_guardian_checked_at: new Date().toISOString(),
                final_guardian_decision: finalDecision,
              })
              .eq('id', item.id);
            ...
            if (finalGuardErr) {
              console.warn(
                '  [Guardian V3] Final pre-send metadata update failed:',
                redactText(finalGuardErr.message),
              );
            }
          } catch (err) {
            console.warn(
              '  [Guardian V3] Final pre-send metadata update exception:',
              redactText(err),
            );
          }
          ...
          if (preSendDelay) {
            ...
            console.log(
              '  [Guardian V3] Mensagem reagendada por cadencia. Pending: ' +
                item.id +
                ' Reason: ' +
                preSendDelay.reasonCode,
            );
            await sleep(500);
            continue;
          }
          ...
          if (!preSendGuardianRun.allow) {
            const reasonCode =
              preSendGuardianRun.blockingDecision?.reason_code ||
              'GUARDIAN_PHASE6_PRE_SEND_BLOCKED';
            ...
            failed++;
            console.warn(
              '  [Guardian V3] Mensagem bloqueada antes do envio. Pending: ' +
                item.id +
                ' Reason: ' +
                reasonCode,
            );
          ...
          if (!finalContentGuardianRun.allow) {
            const reasonCode =
              finalContentGuardianRun.blockingDecision?.reason_code ||
              'GUARDIAN_PHASE5_PRE_SEND_BLOCKED';
            const finalDecision =
              finalContentGuardianRun.blockingDecision?.decision || 'HARD_BLOCK';
            await supabase
              .from('pending_outbound')
              .update({
                failed_at: new Date().toISOString(),
                ...
                final_guardian_decision: finalDecision,
                guardian_config_version_id:
                  item.guardian_config_version_id || finalContentGuardianRun.configVersionId,
              })
              .eq('id', item.id);

            failed++;
            console.warn(
              '  [Guardian V3] Mensagem bloqueada no pre-envio. Pending: ' +
                item.id +
                ' Reason: ' +
                reasonCode,
            );
          ...
          if (limitsExceeded) {
            ...
            console.log(
              '  ⚠️ Limites excedidos para tenant ' +
                tenantId +
                '. Mensagem reagendada para ' +
                newScheduled,
            );
            await sleep(2000);
            continue;
          ...
          if (sendResult.ok) {
            ...
            if (
              ...
              conversation.leads?.status === 'ENRICHED'
            ) {
              await supabase
                .from('leads')
                .update({
              ...
              await supabase.from('lead_events').insert({
                ...
                payload: {
                  from: 'ENRICHED',
                  to: 'CONTACTED',
                  reason: 'Lead contatado pelo Guardião',
                },
                created_at: sendTime,
              });
            }
            ...
            try {
              await supabase.from('whatsapp_guardian_telemetry').insert({
                ...
                number_state: numberState,
                queue_position: 1,
                is_reactive: ['REACTIVE_REPLY', 'CHAT_CONTINUATION', 'LOOKUP_REPLY'].includes(
                  item.message_type || '',
                ),
            ...
            if (failure.critical) {
              console.error(
                '  [WhatsApp Guard] Falha critica no envio para tenant ' +
                  tenantId +
                  '. Motivo: ' +
                  failure.reasonCode,
              );
              ...
              await updateGuardianConnectionState(
                tenantId,
                failure.status,
                healthDecision.externalState,
                failure.reasonCode,
                {
                  locked_at: null,
                  circuit_open_until: new Date(
                    Date.now() + circuitMinutes * 60 * 1000,
                  ).toISOString(),
                },
              );
            ...
            if (failure.shouldBackoff) {
              ...
              failed++;
              console.warn(
                '  [WhatsApp Guard] Envio adiado por falha transitoria. Pending: ' +
                  item.id +
                  ' Reason: ' +
                  failure.reasonCode +
                  ' Retry: ' +
          ...
          await releaseConversationLock(
            ...
            conversationLock.requestedLockUntil,
          );
        }
      } finally {
    ...
    if (targetTenantId) {
      ...
      if (activeCampaigns?.length) {
        tenantIds = Array.from(
          new Set(activeCampaigns.map((c: any) => String(c.tenant_id)).filter(Boolean)),
        );
      }
    }

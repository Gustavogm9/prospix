...
function brtTodayAt(parts: ReturnType<typeof brtParts>, hour: number): string {
  return new Date(
    Date.UTC(parts.year, parts.month - 1, parts.day, hour + BRT_UTC_OFFSET_HOURS, 0, 0, 0),
  ).toISOString();
}
...
async function loadRecentMessages(schedule: Schedule, periodStart: string, periodEnd: string) {
  ...
  let query = supabase
    .from('messages')
    .select(
      'id, tenant_id, conversation_id, direction, sender, content, delivery_status, created_at',
    )
...
async function collectMetrics(schedule: Schedule, periodStart: string, periodEnd: string) {
  ...
  const counts: Record<string, CountResult> = {
    leadsCreated: await safeCount('leadsCreated', 'leads', (q) =>
      applyTenantScope(
        q.gte('created_at', periodStart).lte('created_at', periodEnd).is('deleted_at', null),
        schedule,
      ),
    ),
    ...
    inboundMessages: await safeCount('inboundMessages', 'messages', (q) =>
      applyTenantScope(
        q.eq('direction', 'INBOUND').gte('created_at', periodStart).lte('created_at', periodEnd),
        schedule,
      ),
    ),
    outboundMessages: await safeCount('outboundMessages', 'messages', (q) =>
      applyTenantScope(
        q.eq('direction', 'OUTBOUND').gte('created_at', periodStart).lte('created_at', periodEnd),
        schedule,
      ),
    ),
    pendingDue: await safeCount('pendingDue', 'pending_outbound', (q) =>
      applyTenantScope(
        q.is('sent_at', null).is('failed_at', null).lte('scheduled_for', periodEnd),
        schedule,
      ),
    ),
    ...
    criticalConnectionEvents: await safeCount(
      'criticalConnectionEvents',
    ...
    webhookFailed: await safeCount('webhookFailed', 'evolution_webhook_processing_events', (q) =>
      applyTenantScope(
        q.eq('status', 'FAILED').gte('failed_at', periodStart).lte('failed_at', periodEnd),
        schedule,
      ),
    ),
    webhookStaleProcessing: await safeCount(
      'webhookStaleProcessing',
      'evolution_webhook_processing_events',
      (q) =>
        applyTenantScope(
          q
            .eq('status', 'PROCESSING')
            .lte('processing_started_at', new Date(Date.now() - 5 * 60 * 1000).toISOString()),
          schedule,
        ),
    ),
    webhookDuplicateAttempts: await safeCount(
      'webhookDuplicateAttempts',
      'evolution_webhook_processing_events',
      (q) =>
        applyTenantScope(
          q.gt('attempts', 1).gte('last_seen_at', periodStart).lte('last_seen_at', periodEnd),
          schedule,
        ),
    ),
  };
  ...
  let guardianQuery = supabase
    ...
    .select(
      'tenant_id, status, external_state, last_disconnect_reason_code, updated_at, quarantined_until, circuit_open_until',
    )
  ...
  return {
    ...
    guardianStatus: (guardianStatus || []).map((row: any) => ({
      ...row,
      tenant_name:
        tenantById.get(row.tenant_id)?.name || tenantById.get(row.tenant_id)?.slug || null,
    })),
    aiActivity,
    webhookProcessing,
    recentMessages,
...
async function collectWebhookProcessing(schedule: Schedule) {
  ...
  return {
    issues: issueResult.rows.map((row: any) => ({
      ...
      processing_age_seconds:
        row.processing_age_seconds == null ? null : Number(row.processing_age_seconds),
...
function classifyActivity(input: {
  ...
  const dueAge = input.oldestDuePendingAt
    ? Math.floor((input.nowMs - new Date(input.oldestDuePendingAt).getTime()) / 60000)
    : 0;
  ...
  const inboundAge = input.oldestUnansweredInboundAt
    ? Math.floor((input.nowMs - new Date(input.oldestUnansweredInboundAt).getTime()) / 60000)
    : 0;
  if (input.unansweredConversations > 0 && inboundAge >= 10) {
    return {
      state: 'STALLED',
      summary:
        input.unansweredConversations === 1
  ...
  if (
    ...
    input.duePending > 0 ||
    input.unansweredConversations > 0 ||
    ['COLD', 'RECOVERY', 'HIGH_LOAD', 'COOLDOWN'].includes(guardian)
  ) {
    return {
      state: 'WATCH',
...
async function collectAiActivity(
  schedule: Schedule,
  tenants: any[],
  guardianStatus: any[],
  periodEnd: string,
  ...
  const [pendingResult, unansweredResult, outboundResult, inboundResult, workerSnapshotResult] =
    await Promise.all([
  ...
  const unansweredByTenant = rowsByTenant(
    unansweredResult.rows.filter(
      (row: any) =>
        row.last_inbound_at &&
        (!row.last_outbound_at || row.last_inbound_at > row.last_outbound_at),
    ),
  );
  const outboundByTenant = rowsByTenant(outboundResult.rows);
  const inboundByTenant = rowsByTenant(inboundResult.rows);
  const guardianByTenant = new Map(
    (guardianStatus || []).map((row: any) => [String(row.tenant_id), row]),
  );
  const workerByTenant = new Map(
    (workerSnapshotResult.rows || []).map((row: any) => [String(row.tenant_id), row]),
  );
  ...
  const tenantRows = tenants.map((tenant: any) => {
    ...
    const contactableBacklog = worker
      ? Number(worker.first_touch_eligible || 0)
      : contactable.length;
    ...
    return {
      ...
      oldest_due_age_minutes:
        worker?.oldest_due_age_seconds == null
          ? null
      ...
      guardian_reason_code:
        worker?.guardian_reason_code || guardian?.last_disconnect_reason_code || null,
...
async function processAiActivityAlerts(source: string) {
  ...
  const actionable = activity.tenants.filter((row: any) =>
    ['BLOCKED', 'STALLED'].includes(String(row.state || '').toUpperCase()),
  );
  let sent = 0;
  let failed = 0;
  let skipped = 0;
...
async function createDispatcherRun(mode: string, source: string): Promise<string | null> {
  try {
    ...
    console.warn(
      '[admin-monitoring-dispatcher] run audit insert failed',
      cleanText(err?.message || err, 240),
    );
    return null;
  }
}
...
async function completeDispatcherRun(
  ...
  try {
    ...
    console.warn(
      '[admin-monitoring-dispatcher] run audit update failed',
      cleanText(err?.message || err, 240),
    );
  }
}
...
async function processSchedule(schedule: Schedule, source: string) {
  ...
  const periodStart = new Date(
    Date.now() - Math.max(5, schedule.window_minutes) * 60 * 1000,
  ).toISOString();
...
async function processWebhookReprocess(body: any, source: string) {
  ...
  const { data: event, error: eventError } = await supabase
    ...
    .select(
      'id, event_name, instance_name, status, attempts, payload, accepted_at, processing_started_at, failed_at, updated_at',
    )
  ...
  try {
    ...
    await updateWebhookReprocessRun(runId, accepted ? 'ACCEPTED' : 'FAILED', {
      ...
      error: accepted
        ? null
        : cleanText(parsed?.error || parsed?.message || text || `HTTP ${response.status}`, 500),
    });
  ...
  try {
    ...
    if (mode === 'due') {
      ...
      try {
        ...
        await completeDispatcherRun(
          dispatcherRunId,
          summary.failed > 0 || aiActivityAlerts.failed > 0
    ...
    if (mode === 'webhook_reprocess') {
      ...
      return json(
        result.ok
          ? { ok: true, result }
          : { ok: false, result, error: result.error || 'WEBHOOK_REPROCESS_FAILED' },
        result.ok ? 200 : 400,
      );
    }
